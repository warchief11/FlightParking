...
using FlightParking.Migrations.SeedData;
...
namespace FlightParking.Tests
{
    public abstract class FlightParkingTestBase : AbpIntegratedTestBase<FlightParkingTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FlightParkingTestBase()
        {
        ...
        protected void UsingDbContext(Action<FlightParkingDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FlightParkingDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FlightParkingDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FlightParkingDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FlightParkingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FlightParkingDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FlightParkingDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FlightParkingDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FlightParkingDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FlightParkingDbContext, Task<T>> func)
        {
            T result;
